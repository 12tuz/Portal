package john.fucklocation.portal.ext
...
/**
 * 交通模式 (0-5: STATIONARY, WALKING, RUNNING, CYCLING, DRIVING, HIGH_SPEED)
 */
var Context.transportMode: Int
    get() = sharedPrefs.getInt("transportMode", 1) // 默认WALKING
    set(value) = sharedPrefs.edit {
        putInt("transportMode", value)
    }

/**
 * 步频倍率 (0.5-2.0)
 */
var Context.stepFrequencyMultiplier: Float
    get() = sharedPrefs.getFloat("stepFrequencyMultiplier", 1.0f)
    set(value) = sharedPrefs.edit {
        putFloat("stepFrequencyMultiplier", value)
    }

/**
 * 是否自动检测交通模式
 */
var Context.autoDetectTransportMode: Boolean
    get() = sharedPrefs.getBoolean("autoDetectTransportMode", true)
    set(value) = sharedPrefs.edit {
        putBoolean("autoDetectTransportMode", value)
    }

/**
 * 是否启用传感器/步频模拟
 */
var Context.enableSensorSimulation: Boolean
    get() = sharedPrefs.getBoolean("enableSensorSimulation", true)
    set(value) = sharedPrefs.edit {
        putBoolean("enableSensorSimulation", value)
    }

/**
 * 是否禁用Geocoder反向地理编码
 */
var Context.disableGetFromLocation: Boolean
    get() = sharedPrefs.getBoolean("disableGetFromLocation", false)
    set(value) = sharedPrefs.edit {
        putBoolean("disableGetFromLocation", value)
    }
