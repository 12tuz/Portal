...
object GmsFusedLocationHook : BaseLocationHook() {
    ...
    private fun injectLocationResult(locationResult: Any) {
        // 增强的字段注入策略：尝试多个可能的字段名
        val possibleFieldNames = listOf("mLocations", "zzb", "zzc", "zzd", "locations", "a", "b", "c")
        var injected = false
        ...
        for (fieldName in possibleFieldNames) {
            kotlin.runCatching {
                ...
                if (locations != null && locations.isNotEmpty()) {
                    ...
                    injected = true
                    if (FakeLoc.enableDebugLog) {
                        Logger.debug("GMS LocationResult injected via field: $fieldName")
                    }
                    return
                }
            }
            ...
            kotlin.runCatching {
                locationResult.javaClass.declaredFields.forEach { field ->
                    ...
                    if (value is MutableList<*> && value.isNotEmpty() && value[0] is Location) {
                        ...
                        injected = true
                        if (FakeLoc.enableDebugLog) {
                            Logger.debug("GMS LocationResult injected via field scan: ${field.name}")
                        }
                        return@forEach
                    }
                }
            }.onFailure {
                Logger.error("GMS LocationResult injection failed", it)
            }
        }

        if (!injected && FakeLoc.enableDebugLog) {
            Logger.warn("GMS LocationResult injection: No suitable field found")
        }
    }
}
