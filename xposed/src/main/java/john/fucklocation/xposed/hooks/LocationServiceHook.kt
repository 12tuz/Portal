...
package john.fucklocation.xposed.hooks
...
import john.fucklocation.xposed.RemoteCommandHandler
import john.fucklocation.xposed.hooks.gnss.GnssHook
import john.fucklocation.xposed.hooks.miui.MiuiBlurLocationProviderHook
...
import john.fucklocation.xposed.hooks.nmea.LocationNMEAHook
import john.fucklocation.xposed.hooks.provider.LocationProviderManagerHook
...
internal object LocationServiceHook: BaseLocationHook() {
    // 使用有界队列防止内存溢出，最大512个监听器
    val locationListeners = LinkedBlockingQueue<Pair<String, IInterface>>(512)
    ...
    fun onService(cILocationManager: Class<*>) {
        ...
        cILocationManager.classLoader?.let {
            ...
            Logger.error("ClassLoader is null for $cILocationManager")
            return
        }
        ...
        if(XposedBridge.hookAllMethods(cILocationManager, "registerGnssStatusCallback", object: XC_MethodHook() {
                override fun afterHookedMethod(param: MethodHookParam?) {
                    ...
                    if (cIGnssStatusListener.onceHookAllMethod("onSvStatusChanged", beforeHook {
                        ...
                        // 使用时间依赖波动的卫星数量，更真实地模拟卫星可见性变化
                        val svCount = FakeLoc.getTimeDependentSatelliteCount().coerceAtMost(MAX_SATELLITES)
                        ...
                        if (args[0] != null && args[0].javaClass.name == "android.location.GnssStatus") {
                            runCatching {
                                ...
                                if (mConstructor != null) {
                                    ...
                                    // 降级处理：尝试其他构造函数或保留原始数据
                                    Logger.warn("onSvStatusChanged: constructor with 7 params not found, trying alternatives")
                                    val altConstructor = args[0].javaClass.declaredConstructors.maxByOrNull {
                                        it.parameterTypes.size
                                    }
                                    if (altConstructor == null) {
                                        Logger.error("onSvStatusChanged: no suitable constructor found, keeping original data")
                                    } else {
                                        Logger.debug("onSvStatusChanged: found alternative constructor with ${altConstructor.parameterTypes.size} params")
                                    }
                                }
                            }.onFailure {
                                // 降级处理：构造失败时保留原始 GnssStatus，避免回调收到 null
                                Logger.error("GnssStatus construction failed, keeping original: ${it.message}")
                                if (FakeLoc.enableDebugLog) {
                                    XposedBridge.log(it)
                                }
                            }
